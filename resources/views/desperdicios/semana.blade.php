...
<div class="max-w-6xl mx-auto p-4">
    ...
    <div class="overflow-x-auto rounded shadow-lg border border-gray-300">
        <div class="overflow-x-auto rounded shadow-lg border">
            <div class="min-w-max">
                <table class="w-full bg-white text-sm text-center">
                    <thead class="bg-green-100 text-green-900">
                        <tr>
                            <th class="border px-4 py-3 font-semibold text-left sticky left-0 bg-green-100 z-20 whitespace-nowrap">Turma</th>
                            @foreach(\Carbon\CarbonPeriod::create($inicioSemana, $fimSemana)->filter(fn($date) => !$date->isWeekend()) as $dia)
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-green-50 transition">
                            <td class="border px-4 py-3 font-semibold text-left sticky left-0 bg-white z-10 whitespace-nowrap">
                                {{ $turma->nome }}
                            </td>
                            ...
                            @endphp
                            <td class="border px-4 py-2 whitespace-nowrap align-top">
                                @if($desperdicio)
                                <div class="text-gray-800 leading-relaxed space-y-1 text-left">
                                    ...
                                    <a href="{{ route('desperdicios.edit', $desperdicio) }}"
                                        class="inline-flex items-center gap-1 text-sm text-white bg-green-600 hover:bg-green-700 rounded px-2 py-1 mt-2 transition">
                                        <i class="fa-solid fa-pen-to-square"></i> Editar
                                    </a>
                                </div>
                                @else
                                <a href="{{ route('desperdicios.create', ['turma' => $turma->id, 'data' => $dia->toDateString()]) }}"
                                    class="inline-flex items-center gap-1 text-sm text-green-600 hover:text-green-800 hover:underline">
